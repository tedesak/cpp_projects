...
private:
  ...
  union {
    socow_detail::buffer<T>* _outer_data;
...
private:
  ...
  static void disconnect(socow_detail::buffer<T>* _outer_data, size_t size) {
    _outer_data->disconnect();
    if (_outer_data->_ref_count == 0) {
      remove(_outer_data->_data, _outer_data->_data + size);
      operator delete(_outer_data);
    }
  }
  ...
  void disconnect() {
    if (!is_small_obj) {
      disconnect(_outer_data, _size);
    }
  }

  static void copy_in_range(const T* src, T* dst, size_t start, size_t end) {
    if (end <= start) {
      return;
    }
    std::uninitialized_copy(src + start, src + end, dst + start);
  }

  static void copy_from_begin(const T* src, T* dst, size_t count) {
    copy_in_range(src, dst, 0, count);
  }

  static void remove(T* start, T* end) {
    if (end <= start) {
      return;
    }
  ...
  socow_detail::buffer<T>* create_new_buffer(size_t new_capacity) {
    return new (
        static_cast<socow_detail::buffer<T>*>(operator new(sizeof(socow_detail::buffer<T>) + new_capacity * sizeof(T))))
        socow_detail::buffer<T>(new_capacity);
  }

  socow_vector(const socow_vector& other, size_t capacity, size_t size) : _size(std::min(size, capacity)) {
    if (capacity <= SMALL_SIZE) {
      copy_from_begin(other.data(), _inner_data, _size);
      is_small_obj = true;
    } else {
      try {
        _outer_data = create_new_buffer(capacity);
        copy_from_begin(other.data(), _outer_data->_data, _size);
      } catch (...) {
        operator delete(_outer_data);
        throw;
      }
...
public:
  socow_vector() noexcept : _size(0), is_small_obj(true) {}

  socow_vector(const socow_vector& other) : _size(other._size), is_small_obj(other.is_small_obj) { // O(N) strong
  ...
  socow_vector& operator=(const socow_vector& other) { // O(N) small both: weak; else: strong
    if (&other == this) {
      return *this;
    }
    if (other.is_small_obj) {
  ...
  reference operator[](size_t index) { // O(1)* strong
    return data()[index];
  }
  ...
  iterator begin() { // O(1)* strong
    return data();
  }

  iterator end() { // O(1)* strong
    return data() + _size;
  }
  ...
  void shrink_to_fit() { // O(N) small: nothrow; else: strong
  ...
  void swap(socow_vector& other) {
    ...
    if (is_small_obj && other.is_small_obj) {
      ...
      socow_detail::buffer<T>* tmp = other._outer_data;
  ...
  void push_back(const T& value) { // O(1)* strong
    if (capacity() == _size || (!is_small_obj && _outer_data->is_shared())) {
      socow_vector tmp(*this, _size ? (capacity() == 0 ? 1 : capacity() * 2) : capacity());
      tmp.push_back(value);
      *this = tmp;
    } else {
  ...
  void pop_back() { // O(1)* unshared_big,small: nothrow; shared_big: strong
    if (!is_small_obj && _outer_data->is_shared()) {
  ...
  iterator insert(const_iterator pos, const T& value) { // O(N) strong
    ...
    if (capacity() == _size || (!is_small_obj && _outer_data->is_shared())) {
      socow_vector tmp(*this, capacity() == 0 ? 1 : capacity() * 2, shift);
      tmp.push_back(value);
      copy_in_range(is_small_obj ? _inner_data : _outer_data->_data, tmp._outer_data->_data + 1, shift, _size);
      tmp._size += _size - shift;
      clear();
      *this = tmp;
      return _outer_data->_data + shift;
    }
    push_back(value);
  ...
  iterator erase(const_iterator pos) { // O(N) unshared_big,small: nothrow; shared_big: strong
    return erase(pos, pos + 1);
  }

  iterator erase(const_iterator first, const_iterator last) { // O(N) unshared_big,small: nothrow; shared_big: strong
    ptrdiff_t shift_first = first - std::as_const(*this).begin(), shift = last - first;
    if (first == last) {
      return const_cast<iterator>(first);
    }
    if (!is_small_obj && _outer_data->is_shared()) {
      socow_vector tmp(*this, capacity(), shift_first);
      copy_in_range(_outer_data->_data + shift, tmp._outer_data->_data, shift_first, _size - shift);
      tmp._size = _size - shift;
      clear();
      *this = tmp;
      return _outer_data->_data + shift_first;
    }
